...
class LetShoutSpec extends WordSpec with Matchers with ScalatestRouteTest with LetShoutRoute with TwitterMock {
  ...
  "The LetShoutSpec" should {
    ...
    "return a greeting for GET requests to the root path" in {
      val tweetHandlerRouter = system.actorOf(RoundRobinPool(5) props (TweetsActor.props()), "tweetHandlerRouter")
      Get("/v1/letshout?user=user&tweets=10") ~> route ~> check {
